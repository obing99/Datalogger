...
-Clone Racedac output string to log on Racechrono/Dynojet/
-write software to analyze
...
9.8.2015 - 0.0.7 - Added Batterie Voltage
 */
  ...
  if (k_mode == 1) {                                                      // Run over Sentence Request
    if (k_transmit(K_READ_ALL_SENS, 7)) {
      ...
      if (!sd.exists(FILENAME)) {
        ...
        file.println(F("Time(ms),AFR,RPM,TPS,ECT,IAT,IAP,Voltage,Gear,Fuel 1,Fuel 2,Fuel 3,Fuel 4,IGN 1,IGN 2,IGN 3,IGN 4,STVP,Pair,Clutch,Map,Neutral,NMEA Type,Time,Status,Latitude,North South,Longitude,East West,Speed,Track Angle,Date,Magnetic Variation,,Checksum"));
        file.flush();
        file.close();
      }
    }
